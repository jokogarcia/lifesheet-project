...
export function CVPreviewer({ cvData, options, printMode }: CVPreviewerProps) {



    const template = options.template || "single-column-1";
    console.log("Rendering", options.template)
    const cssVars = getStyleOverrides(options);

    return <div className={moduleStyles.reset}>
        <div className={`${moduleStyles.rootContainer} ${printMode ? moduleStyles.printable : ""}`}
            style={cssVars}>
...
export function renderTwoColumns({ cvData, options, printMode }: CVPreviewerProps) {
    ...
    return (<div
        className={`${moduleStyles.container} ${moduleStyles['two-column']} ${printMode ? moduleStyles.printable : ""}`}>
        <div className={`${moduleStyles['left-panel']}`}>
            {cvData.personal_info.profilePictureUrl ?
                <div className={`${moduleStyles['profile-photo']}`}>
        ...
        <div className={`${moduleStyles['right-panel']}`}>
            ...
            <div className={`${moduleStyles.section}`}>
                ...
                <ul className={`${moduleStyles['skills-list']}`}>
                    {cvData.skills.map(skill => (
                        <li key={skill.id}>{skill.name}</li>
                    ))}
                </ul>
            </div>
            <div className={`${moduleStyles.section} ${moduleStyles.breakable}`}>
                <h2>{cvData.workExperienceTitle || "Work Experience"}</h2>
                {cvData.work_experience.map(job => (
                    <div className={`${moduleStyles['work-experience']}`} key={job.id}>
                        <h3 className={`${moduleStyles['we-jobtitle']}`}>{job.position}</h3>
                        <span className={`${moduleStyles['we-company']}`}>{job.company}</span>

                        <span className={`${moduleStyles['we-dates']}`}>{job.startDate} - {job.endDate}</span>
                        <h5 className={`${moduleStyles['we-location']}`}>📍 {job.location}</h5>
                        <p>{job.description}</p>
...
export function renderOneColumn({ cvData, options, printMode }: CVPreviewerProps) {
    ...
    return (<div
        className={`${moduleStyles.container} ${moduleStyles['one-column']} ${printMode ? moduleStyles.printable : ''}`}>
        {cvData.personal_info.profilePictureUrl ?
            <div className={`${moduleStyles['profile-photo']} profile-photo`}>
                <img src={cvData.personal_info.profilePictureUrl} alt="Profile" />
            </div>
            : ''}
        <div className={`${moduleStyles.header}`}>
            <div className={`${moduleStyles['profile-info']}`}>
                <h1>{cvData.personal_info.fullName}</h1>
                <h2 className={`${moduleStyles['job-title']}`}>{cvData.personal_info.title}</h2>
                <div className={`${moduleStyles['contact-info']}`}>
            ...
            {cvData.personal_info.profilePictureUrl &&
                <div className={`${moduleStyles['profile-photo']} ${moduleStyles['square']}`}>
                    <img className={`${moduleStyles['profile-photo']}`} src={cvData.personal_info.profilePictureUrl} alt="Profile" />
                </div>}
        </div>
        <div className={`${moduleStyles.section}`}>
            <h2>{cvData.summaryTitle || "Summary"}</h2>
