        ...
        )}
        {
          cvData.tailored?.sectionOrder && cvData.tailored?.sectionOrder.map(sectionKey => {
            ...
            switch (sectionKey) {
              ...
              case 'personalInfo':
                return renderPersonalInfo(cvData, moduleStyles);
              case 'summary':
                return renderSummarySection(cvData, moduleStyles);
              case 'skills':
                return renderSkillsSection(cvData, moduleStyles);
              case 'workExperience':
                return renderWorkExperienceSection(cvData, moduleStyles);
              case 'education':
                return renderEducationSection(cvData, moduleStyles);
              case 'languages':
                return renderLanguagesSection(cvData, moduleStyles);
              default:
                return null;
            }
          })
      ...
      <div className={`${moduleStyles.header} header`}>
        <div className={`${moduleStyles['profile-info']} profile-info`}>
          ...
          <div className={`contact-info ${moduleStyles['contact-info']}`}>
            {cvData.personal_info.email && options.includeEmail && (
              <p>
                Email:{' '}
                <a href={`mailto:${cvData.personal_info.email}`}>{cvData.personal_info.email}</a>
              </p>
            )}
            {cvData.personal_info.phone && options.includePhone && (
              <p>
                Phone:{' '}
                <a href={`tel:${cvData.personal_info.phone}`}>{cvData.personal_info.phone}</a>
              </p>
            )}
            {cvData.personal_info.location && options.includeAddress && (
              <p>
                Location:{' '}
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${cvData.personal_info.location}`}
                >
            ...
            {cvData.personal_info.linkedIn && (
              <p>
                LinkedIn:{' '}
                <a href={cvData.personal_info.linkedIn}>{cvData.personal_info.linkedIn}</a>
              </p>
            )}
            {cvData.personal_info.github && (
              <p>
                GitHub: <a href={cvData.personal_info.github}>{cvData.personal_info.github}</a>
              </p>
            )}
          </div>
        </div>
        {cvData.personal_info.profilePictureUrl && (
          <div
            className={`profile-photo square ${moduleStyles['profile-photo']} ${moduleStyles['square']}`}
          >
            <img
              className={`profile-photo ${moduleStyles['profile-photo']}`}
              src={cvData.personal_info.profilePictureUrl}
              alt="Profile"
            />
          </div>
        )}
      ...
      {cvData.tailored?.sectionOrder && cvData.tailored?.sectionOrder.map(sectionKey => {
        ...
        switch (sectionKey) {
          case 'cover-letter':
            return null; // Cover letter is rendered above the columns
          case 'personalInfo':
            return null; // Personal info is already rendered in the header
          case 'summary':
            return renderSummarySection(cvData, moduleStyles);
          case 'skills':
            return renderSkillsSection(cvData, moduleStyles);
  ...
  return <div className={`section breakable ${moduleStyles.section} ${moduleStyles.breakable}`}>
    ...
    {cvData.work_experience.map(job => (
      <div className={`work-experience ${moduleStyles['work-experience']}`} key={job.id}>
        ...
        <span className={`we-dates ${moduleStyles['we-dates']}`}>
...
const renderPersonalInfo = (cvData: CVDataWithTitle, moduleStyles: any) => {
  return <div className={`section ${moduleStyles.section}`}>
    ...
    {cvData.personal_info.phone && <p><strong>Phone:</strong> {cvData.personal_info.phone}</p>}
    {cvData.personal_info.location && <p><strong>Location:</strong> {cvData.personal_info.location}</p>}
    {cvData.personal_info.linkedIn && <p><strong>LinkedIn:</strong> {cvData.personal_info.linkedIn}</p>}
    {cvData.personal_info.github && <p><strong>GitHub:</strong> {cvData.personal_info.github}</p>}
  </div>;
}