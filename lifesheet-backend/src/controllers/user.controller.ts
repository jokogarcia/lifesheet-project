...
export const tailorCV = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!jobDescription && !jobDescriptionId) {
      throw new ApiError(
        400,
        'Job description is required. Provide one or a valid job description ID.'
      );
    }
    if (!jobDescription) {
    ...
    const j = await TailorCVQueue.add(`${jobDescriptionId}-${Date.now()}`, {
      userId,
      jobDescriptionId,
      companyName,
      includeCoverLetter,
...
export const getUsersActiveSubscription = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const userId = resolveUserId(req);
    const now = new Date();
    const { usersConsumptions, todaysConsumptions, thisWeeksConsumptions } =
      await getUsersConsumptions(userId);
...
export const getUsersSubscriptionStatus = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const initiatePlanPurchase = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { provider, planId, successUrl, cancelUrl } = req.body as {
    ...
    if (provider === 'stripe') {
      ...
      const session = await stripeClient.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url:
          successUrl + `?session_id={CHECKOUT_SESSION_ID}&subscription_id=${s._id.toString()}`,
