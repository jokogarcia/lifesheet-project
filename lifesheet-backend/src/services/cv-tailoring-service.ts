...
export async function tailorCV(
  ...
  if (!useAiTailoring) {
    ...
    return {
      tailored_cv: { ...cvObject, created_at: new Date(), updated_at: new Date(), _id: undefined },
      tokens_used: 0,
    };
  }
  ...
  if (!GEMINI_API_KEY) {
    throw new Error(
      'GEMINI_API_KEY is not set. Configure constants.GEMINI_API_KEY before calling tailorCV.'
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (!isValidCV(tailored)) {
        if (attempts++ >= 2) throw new Error('Invalid CV structure returned from model');
...
export async function generateCoverLetter(
  ...
  if (!GEMINI_API_KEY) {
    throw new Error(
      'GEMINI_API_KEY is not set. Configure constants.GEMINI_API_KEY before calling generateCoverLetter.'
    );
  }
